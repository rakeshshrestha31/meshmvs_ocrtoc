...
logger = logging.getLogger("ocrtoc")
P2M_SCALE = 1.0
...
def main_worker_eval(worker_id, args):
    ...
    # debug_eval(model, test_loader, cfg, device)
...
@torch.no_grad()
def debug_eval(model, loader, cfg, device):
    for i, batch in enumerate(loader):
        ...
        debug_batch(batch, model, model_kwargs, loader)
...
def training_loop(cfg, cp, model, optimizer, scheduler, loaders, device, loss_fn):
    ...
    while cp.epoch < cfg.SOLVER.NUM_EPOCHS:
        ...
        for i, batch in enumerate(loaders["train"]):
            ...
            # debug_batch(batch, model, model_kwargs)
            # exit(0)
...
@torch.no_grad()
def debug_batch(batch, model, model_kwargs, loader=None):
    # depths, masked_depths = model.predict_depths(
    #     imgs=batch["imgs"], **model_kwargs
    # )

    # voxels = model.get_voxels_from_depths(
    #     masked_depths, batch["intrinsics"], batch["extrinsics"]
    # )
    # voxels, depth_points = voxels["voxel_scores"], voxels["depth_clouds"]

    # from shapenet.modeling.voxel_ops import cubify
    # cubified = cubify(
    #     voxels, model.voxel_size, model.cubify_threshold
    # )

    model_output = model(batch["imgs"], **model_kwargs)
    batch_size, num_views = batch["imgs"].shape[0:2]
    ...
    def pytorch3d_mesh_to_o3d(pytorch3d_mesh):
    ...
    for batch_idx in range(batch_size):
        cubified_vertices = cubified[batch_idx].verts_packed().detach().cpu().numpy()
        cubified_faces = cubified[batch_idx].faces_packed().detach().cpu().numpy()
        ...
        if (batch.get("points", None) is not None) \
            ...
            gt_pcd = o3d.geometry.PointCloud(o3d.utility.Vector3dVector(
                batch["points"][batch_idx].detach().cpu().numpy()
            ))
            gt_pcd.normals = o3d.utility.Vector3dVector(
                batch["normals"][batch_idx].detach().cpu().numpy()
            )
        ...
        if batch.get("meshes", None) is not None:
            ...
            o3d.io.write_triangle_mesh(f"gt_mesh_{batch_idx}.ply", gt_mesh)
        ...
        if (batch.get("id_strs", None) is not None) and (loader is not None):
            ...
            T_cam0_world = batch["extrinsics"][batch_idx, 0].detach().cpu().numpy()
            data_dir = loader.dataset.data_dir
            metadata = loader.dataset.read_camera_parameters(data_dir, scene_name)
            gt_mesh_textured = o3d.io.read_triangle_mesh(os.path.join(
                data_dir, "models", metadata["object_name"], "visual.ply"
            )).transform(T_cam0_world)
            geometries.append(gt_mesh_textured)
            o3d.io.write_triangle_mesh(
                f"gt_mesh_textured_{batch_idx}.ply", gt_mesh_textured
            )
        ...
        for view_idx in range(num_views):
            if model_output.get("depth_clouds", None) is not None :
                pcd = o3d.geometry.PointCloud(o3d.utility.Vector3dVector(
                    model_output["depth_clouds"][batch_idx][view_idx].detach().cpu().numpy()
                ))
            ...
            if batch.get("voxels", None) is not None :
                gt_cubified = cubify(
                    logit(batch["voxels"][batch_idx, view_idx, ...].unsqueeze(0).float() + 1e-3),
                    model.voxel_size, model.cubify_threshold
                )
                cubified_vertices = gt_cubified[0].verts_packed().detach().cpu().numpy()
                cubified_faces = gt_cubified[0].faces_packed().detach().cpu().numpy()
...
def setup(args):
    ...
    # register dataset
    data_dir = register_ocrtoc(cfg.DATASETS.NAME)
    ...
    logger = setup_logger(
        output_dir, color=colorful_logging, name="ocrtoc", distributed_rank=comm.get_rank()
    )
    logger.info(
