...
from shapenet.evaluation import evaluate_split_depth, evaluate_test, evaluate_test_p2m
...
from shapenet.modeling.heads import MVSNet
...
def main_worker(worker_id, args):
    ...
    def loss_fn(depth_gt, depth_est, mask):
        nonlocal cfg
        return adaptive_berhu_loss(depth_gt, depth_est, mask) \
                    * cfg.MODEL.MVSNET.PRED_DEPTH_WEIGHT
...
def eval_and_save(model, loaders, optimizer, scheduler, cp):
    ...
    if comm.is_main_process():
        ...
        train_metrics, train_preds = evaluate_split_depth(
            model, loaders["train_eval"], prefix="train_", max_predictions=10 # 1000
        )
        ...
        test_metrics, test_preds = evaluate_split_depth(
            model, loaders[eval_split], prefix="%s_" % eval_split, max_predictions=100 # 1000
        )
        ...
        cp.early_stop_metric = eval_split + "_negative_depth_loss"
